...
#
# Returns the basic NWModel.  Inherits your model from here.
#
# Here is a sample for how to use it as sails model.
#
...
define module, [
  'underscore', 'async', 'path', 'mongoose', 'change-case', 'sails', 'mongodb'
], (
  _, async, path, mongoose, changeCase, sails, mongodb
) ->

  getMongooseDBConnection = _.memoize (adapter) ->
    config = sails.config.adapters[adapter]
    db = mongoose.createConnection()
    db.open config.host, config.database, config.port

  __mongodbCache = {}

  getMongoDB = (adapter, cb) ->
    if __mongodbCache[adapter]? then return cb null, __mongodbCache[adapter]
    config = sails.config.adapters[adapter]
    db = new mongodb.Db(adapter, new mongodb.Server(config.host, config.port), safe: true)
    db.open ->
  ...
  # Base NWModel.  Which provides the ability to use mongoose's schema management and sails model
  # framework.
  class NWModel
